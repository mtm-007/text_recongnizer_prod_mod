"""Experiment-runining Trainig Framework"""
import argparse
import importlib
import numpy as np
import torch
...
def _setup_parser():
    ...
    module_class.add_to_argparse(model_group)
...
def main():
    ...
    model_checkpointing_callback = pl.callbacks.ModelCheckpoint(
        filename = "{epoch: 03d}-{val_loss:.3f}={val-cer:.3f}", monitor="val_loss", mode="min"
    )
