...
from text_recognizer.datasets.emnist_line_dataset import EmnistDataset
...
class LineModel(Model):
    ...
    def __init__(self,
                 ...
                 datasets_args: Dict= None,
                 network_args: Dict= None):
        """Define the default dataset and network values for this model."""
        super().__init__(dataset_cls, network_fn, datasets_args, network_args)
    ...
    def evaluate(self, x, y, batch_size=16, verbose=False):
        ...
        if verbose:
            sorted_inds = np.argsort(char_accuracies)
    ...
    def predict_on_image(self, image: np.ndarray) -> Tuple[str, float]:
        ...
        conf = np.min(np.max(pred_raw, axis=-1))
        return pred, conf
