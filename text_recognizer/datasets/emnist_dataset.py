"""Emnist Dataset. Download from NIST website and saves as .npz file if not already present.
"""
...
def _download_and_process_emnist():
    ...
    # os.chdir(curdir)
    metadata = toml.load(METADATA_FILENAME)
    dataset_metadata = metadata['dataset']  # Access the 'dataset' section
    curdir = os.getcwd()
    os.chdir(RAW_DATA_DIRNAME)
    _download_raw_dataset(dataset_metadata)  # Pass the correct metadata
    _process_raw_dataset(dataset_metadata['filename'])
    os.chdir(curdir)
...
def _sample_to_balance(x, y):
    ...
    for label in np.unique(y.flatten()):
        ...
        all_sampled_ids.append(sampled_inds)
    ind = np.concatenate(all_sampled_ids)
    x_sampled = x[ind]
    y_sampled = y[ind]
