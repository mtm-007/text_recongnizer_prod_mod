"""Base Dataset class."""
from typing import Tuple, Any, Callable, Dict, Sequence, Union
import torch
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def _get_item(self, index: int) ->Tuple[Any, Any]:
        """
...
def convert_string_to_labels(strings: Sequence[str], mapping: Dict[str, int], length: int) ->torch.Tensor:
    """
...
def split_dataset(base_dataset: BaseDataset, fraction: float, seed: int)->Tuple[BaseDataset, BaseDataset]:
    ...
    return torch.utils.data.random_split(
        base_dataset, [split_a_size, split_b_size], generator = torch.Generator().manual_seed()
    )
